...
export default function BlogPaginate({ datas }) {
    ...
    const { data: tags } = useSWR(`${process.env.NEXT_PUBLIC_BASE_URL}${CATEGORIES}`, fetcher, {
        ...
        <div >
            ...
            {data?.meta?.pagination?.pageCount > 1 ? (
                <Suspense fallback={<div>Loading pagination...</div>}>
                    <Pagination page={page} setPage={setPage} pageCount={data?.meta?.pagination?.pageCount} />
                </Suspense>
            ) : ""}
        </div>
    );
