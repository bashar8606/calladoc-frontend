    ...
    >
      ...
      <div className="container">
        <div className=" lg:mb-14">
          ...
          <h3 className="text-sky-900 text-center mx-auto font-dmSerif  text-5xl xl:text-6xl font-bold  max-w-[14ch]">
            {data?.titleMain}
          </h3>
        </div>
        <Card className="w-full h-full bg-white grid gap-4 md:gap-8 grid-cols-1 md:grid-cols-2 mx-auto overflow-hidden  max-w-2xl relative bg-white/30 border border-[#d4e1ff] shadow-[1px_3px_11px_0px_#dbe6ff] backdrop-blur-[12px] rounded-[36px] p-[30px]">
            ...
            <div className="md:block   mb-6  justify-center ">
              {data?.items?.map((item, i) => {
                return (
                  <div
                    key={i}
                    className={`border border-[#d4e1ff] p-3 mb-3 bg-[linear-gradient(303deg,#2463eb_65%,#abc2f6_102%)] z-[1] rounded-[20px] relative shadow-[0px_0px_10px_1px_#2463eb0f] ${
          ...
          <div>
            {data?.items[0]?.avail && (
              <>
                ...
                <p className="text-gray-700 text-sm flex-1 mb-7">
                  {data?.items[0]?.avail ||
                    "Type 2 Diabetes, Prediabetes, Gestational Diabetes Mellitus (GDM), Insulin Resistance."}
                </p>
              </>
            )}
            ...
            <p className="text-gray-500 text-sm flex-1 ">
              {data?.items[0]?.note ||
                "Customization (if required) Lab services screening Medical Assessment devices etc"}
            </p>)}
          </div>
        </Card>
